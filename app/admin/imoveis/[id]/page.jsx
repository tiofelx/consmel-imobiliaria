...
export default function EditProperty({ params }) {
  ...
  const handleCepBlur = async () => {
    ...
    if (data) {
      setAddress(prev => ({ ...prev, street: data.street, neighborhood: data.neighborhood, city: data.city, state: data.state }));
    }
  };
  ...
  return (
    <>
      <AdminHeader
        ...
        subtitle="Atualize as informações, valores e mídia do imóvel."
          ...
          <div className="form-grid">
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <div className="input-wrapper">
                <input type="text" className="form-input" value={formData.title} onChange={(e) => handleChange('title', e.target.value)} placeholder="Ex: Casa de Alto Padrão no Jardins" />
              </div>
            </div>
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              <label className="form-label">Descrição</label>
              <textarea className="form-textarea" value={formData.description} onChange={(e) => handleChange('description', e.target.value)} placeholder="Descreva os detalhes..." rows="4"></textarea>
            </div>
        ...
        {currentStep === 5 && (
          <div>
            ...
            <div className="dropzone" onClick={() => fileInputRef.current?.click()} style={{ cursor: isProcessing ? 'wait' : 'pointer' }}>
              ...
              <p style={{ fontWeight: 600, fontSize: '1.1rem' }}>{isProcessing ? 'Processando...' : 'Clique para selecionar fotos'}</p>
            </div>
            {images.length > 0 && (
