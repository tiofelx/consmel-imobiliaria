...
export async function createSession(payload, duration = '24h', cookieName = 'session') {
    ...
    cookieStore.set(cookieName, session, {
        ...
        expires,
        // 'lax' (não 'strict') porque OAuth flows envolvem redirect
        // a partir de origem cross-site (accounts.google.com). Strict
        // bloqueia o cookie nesse primeiro hop e o usuário cai logado
        // mas sem cookie ativo. Lax mantém proteção CSRF (cookie não
        // viaja em POST/DELETE cross-site) e permite navegação top-level.
        sameSite: 'lax',
        path: '/',
    });
